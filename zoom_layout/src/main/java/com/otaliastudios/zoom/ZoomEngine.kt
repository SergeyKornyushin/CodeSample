...
import com.otaliastudios.common.extensions.getLocationOnScreen
...
constructor(context: Context): ZoomApi {
    ...
    private inner class Callbacks: ViewTreeObserver.OnGlobalLayoutListener,
        ...
        override fun onMatrixSizeChanged(oldZoom: Float, firstTime: Boolean) {
            LOG.w(
                "onMatrixSizeChanged: firstTime:", firstTime, "oldZoom:", oldZoom,
            ...
            LOG.i(
                "onMatrixSizeChanged: newTransformationZoom:", zoomManager.transformationZoom,
    ...
    private var zoomChangedAction: (Float) -> Unit = {}
    ...
    private val matrixController: MatrixController =
        MatrixController(zoomManager, panManager, stateController, callbacks)

    // Gestures
    private val scrollFlingDetector =
        ScrollFlingDetector(context, panManager, stateController, matrixController)

    private val pinchDetector =
        PinchDetector(context, zoomManager, panManager, stateController, matrixController)
    ...
    /**
     * Returns the container central point by x coordinate.
     * @return container central x coordinate
     */
    private val containerCenterX get() = containerWidth / 2f

    /**
     * Returns the container central point by y coordinate.
     * @return container central y coordinate
     */
    private val containerCenterY get() = containerHeight / 2f
     ...
     */
    @Deprecated(
        "Deprecated in favor of not using RectF class",
    ...
    private fun computeTransformationGravity(input: Int): Int {
        return when (input) {
            ZoomApi.TRANSFORMATION_GRAVITY_AUTO -> {
                val horizontal =
                    Alignment.toHorizontalGravity(panManager.alignment, Gravity.CENTER_HORIZONTAL)
                val vertical =
                    Alignment.toVerticalGravity(panManager.alignment, Gravity.CENTER_VERTICAL)
    ...
    override fun zoomToPoint(targetZoom: Float, event: MotionEvent, needToAnimate: Boolean) {
        val zoomFactor = targetZoom / zoom
        zoomChangedAction.invoke(targetZoom)

        val handler = ZoomToPointHandler(zoomFactor = zoomFactor, event = event)

        MatrixUpdate.obtain {
            this.needEnsurePan = handler.needEnsurePan(targetZoom = targetZoom)
            pivot(pivotX = handler.getPivotX(), pivotY = handler.getPivotY())
            zoomTo(zoom = targetZoom, overZoom = false)
        }.apply { updateMatrix(update = this, animate = needToAnimate) }
    }

    override fun maxZoomToPoint(event: MotionEvent, needToAnimate: Boolean) {
        zoomToPoint(targetZoom = getMaxZoom(), event = event, needToAnimate = needToAnimate)
    }
     ...
     */
    override fun zoomIn(factor: Float) {
        zoomBy(zoomFactor = factor, animate = true)
        zoomChangedAction.invoke(zoom * factor)
    }

    /**
     * Applies a small, animated zoom-out.
     */
    override fun zoomOut(factor: Float) {
        zoomBy(zoomFactor = factor, animate = true)
        zoomChangedAction.invoke(zoom * factor)
    }
    ...
    override fun setOnZoomChangedListener(zoomChangedAction: (Float) -> Unit) {
        this.zoomChangedAction = zoomChangedAction
        pinchDetector.setOnZoomChangedListener(zoomChangedAction)
    }
    //endregion
    ...
    private fun updateMatrix(update: MatrixUpdate, animate: Boolean) {
        if (animate) {
            matrixController.animateUpdate(update)
        } else {
            cancelAnimations()
    ...
    private inner class ZoomToPointHandler(
        private val zoomFactor: Float,
        private val event: MotionEvent
    ) {

        private val containerLocation = container.getLocationOnScreen()
        private val realX = getContainerCoordinate(axis = CoordinateAxis.X)
        private val realY = getContainerCoordinate(axis = CoordinateAxis.Y)

        fun getPivotX(): Float {
            val axis = CoordinateAxis.X

            val isClickByLeftSide = isClickBeforeCenter(axis = axis)
            val edgeX = getEdgeCoordinate(axis = axis, isClickBeforeCenter = isClickByLeftSide)
            val distanceX = calculateEdgeToPointDistance(coordinate = realX, edge = edgeX)
            val edgeBiasX = calculateEdgeBias(distance = distanceX, center = containerCenterX)

            return calculatePivot(
                coordinate = realX,
                center = containerCenterX,
                isClickBeforeCenter = isClickByLeftSide,
                edgeBias = edgeBiasX
            )
        }

        fun getPivotY(): Float {
            val axis = CoordinateAxis.Y

            val isClickByTopSide = isClickBeforeCenter(axis = axis)
            val edgeY = getEdgeCoordinate(axis = axis, isClickBeforeCenter = isClickByTopSide)
            val distanceY = calculateEdgeToPointDistance(coordinate = realY, edge = edgeY)
            val edgeBiasY = calculateEdgeBias(distance = distanceY, center = containerCenterY)

            return calculatePivot(
                coordinate = realY,
                center = containerCenterY,
                isClickBeforeCenter = isClickByTopSide,
                edgeBias = edgeBiasY
            )
        }

        /**
         * Вычислить нужно ли включить автоцентрирование контента при зуме.
         * Должен быть активен если увеличенный контент меньше по размеру чем контейнер.
         */
        fun needEnsurePan(targetZoom: Float): Boolean {
            return when (contentWidth / contentHeight > 1) {
                false -> contentWidth * targetZoom < containerWidth
                true -> contentHeight * targetZoom < containerHeight
            }
        }

        /**
         * Вычисление отступов начальных координат учитывая отступы контейнера
         */
        private fun getContainerCoordinate(axis: CoordinateAxis): Float {
            return when (axis) {
                CoordinateAxis.X -> event.rawX - containerLocation[0]
                CoordinateAxis.Y -> event.rawY - containerLocation[1]
            }
        }

        /**
         * Вычисление смещения матрицы преобразования в центр экрана после скейла.
         * pivot formula: trans = (1 - scale) * pivot
         */
        private fun calculateMatrixPivot(coordinate: Float, center: Float): Float =
            (coordinate * zoomFactor - center) / (zoomFactor - 1)

        /**
         * Определяет с какой стороны контейнера был клик.
         *
         * @return true - перед центром, иначе - false
         */
        private fun isClickBeforeCenter(axis: CoordinateAxis): Boolean {
            return when (axis) {
                CoordinateAxis.X -> realX < containerCenterX
                CoordinateAxis.Y -> realY < containerCenterY
            }
        }

        /**
         * Возвращает координаты ближайшей к клику стороны.
         */
        private fun getEdgeCoordinate(axis: CoordinateAxis, isClickBeforeCenter: Boolean): Float {
            val pan = if (axis == CoordinateAxis.X) scaledPanX else scaledPanY
            val contentSize = if (axis == CoordinateAxis.X) contentWidth else contentHeight

            return if (isClickBeforeCenter) pan else pan + contentSize * zoom
        }

        /**
         * Вычисление расстояния от точки нажатия до края после скейла.
         */
        private fun calculateEdgeToPointDistance(coordinate: Float, edge: Float): Float =
            when (coordinate < edge) {
                true -> (edge - coordinate) * zoomFactor
                false -> (coordinate - edge) * zoomFactor
            }

        /**
         * Вычислить отступ после смещения матрицы в центр, чтобы прижать контент к краю.
         */
        private fun calculateEdgeBias(distance: Float, center: Float): Float =
            when (distance < center) {
                true -> (center - distance) / (zoomFactor - 1)
                false -> 0f
            }

        /**
         * Вычисление смещения матрицы преобразования таким образом,
         * что точка нажатия после скейла перемещается к центру,
         * а край контента прижимается к краю контейнера если находится внутри его.
         */
        private fun calculatePivot(
            coordinate: Float,
            center: Float,
            isClickBeforeCenter: Boolean,
            edgeBias: Float
        ): Float {
            val scalePivot = calculateMatrixPivot(coordinate = coordinate, center = center)

            return if (isClickBeforeCenter) scalePivot + edgeBias else scalePivot - edgeBias
        }
    }

    private enum class CoordinateAxis { X, Y }
}
