...
internal class MatrixController(
    ...
    private fun onSizeChanged(oldZoom: Float, forceReset: Boolean) {
        ...
        if (contentWidth <= 0
            ...
            return
        LOG.w(
            "onSizeChanged:", "containerWidth:", containerWidth,
    ...
    internal fun applyUpdate(update: MatrixUpdate) {
        ...
        if (update.needEnsurePan) ensurePan(update.canOverPan)
    ...
    internal fun animateUpdate(update: MatrixUpdate) {
        ...
        if (update.pan != null) {
            ...
            val target =
                if (update.isPanRelative) scaledPan + update.scaledPan else update.scaledPan
            // ofObject doesn't respect animator.interpolator, so we use ofFloat instead
        ...
        animator.addUpdateListener {
            applyUpdate {
                ...
                needEnsurePan = update.needEnsurePan
