...
interface ZoomApi {
    ...
    /**
     * Zoom to the [MotionEvent] coordinates.
     *
     * @param event   event with coordinates
     * @param targetZoom target zoom value
     * @param needToAnimate whether to animate the transition
     */
    fun zoomToPoint(targetZoom: Float, event: MotionEvent, needToAnimate: Boolean)

    /**
     * Max zoom to the [MotionEvent] coordinates by [ZoomApi.zoomToPoint].
     *
     * @param event   event with coordinates
     * @param needToAnimate whether to animate the transition
     */
    fun maxZoomToPoint(event: MotionEvent, needToAnimate: Boolean)
     ...
     */
    fun zoomIn(factor: Float = ZOOM_IN_FACTOR)

    /**
     * Applies a small, animated zoom-out.
     */
    fun zoomOut(factor: Float = ZOOM_OUT_FACTOR)
    ...
    /**
     * External listener of zoom changing
     */
    fun setOnZoomChangedListener(zoomChangedAction: (Float) -> Unit)

    companion object {

        val ZOOM_IN_FACTOR get() = 1.5f
        val ZOOM_OUT_FACTOR get() = 0.5f
